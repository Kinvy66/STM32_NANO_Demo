...
#define LCD_SCK_PIN      		 LCD_SCL_Pin									// SCK引脚， 需要重定义SPI3的IO口复用
#define LCD_SCK_PORT     		 LCD_SCL_GPIO_Port                 					// SCK引脚用到的端口
#define GPIO_LCD_SCK_CLK      __HAL_RCC_GPIOA_CLK_ENABLE()  		// SCK引脚IO口时钟

#define LCD_SDA_PIN      		 LCD_SDA_Pin									// SDA引脚， 需要重定义SPI3的IO口复用
#define LCD_SDA_PORT    		 LCD_SDA_GPIO_Port                 					// SDA引脚用到的端口
#define GPIO_LCD_SDA_CLK      __HAL_RCC_GPIOA_CLK_ENABLE()  		// SDA引脚IO口时钟
...
#define GPIO_LCD_DC_CLK     			__HAL_RCC_GPIOA_CLK_ENABLE()	// 数据指令选择  GPIO时钟

#define LCD_Backlight_PIN				LCD_BL_Pin				         // 背光  引脚
#define	LCD_Backlight_PORT			LCD_BL_GPIO_Port									// 背光 GPIO端口
#define GPIO_LCD_Backlight_CLK     __HAL_RCC_GPIOA_CLK_ENABLE()	// 背光 GPIO时钟
